...
| `sf_criminal_hf` | CA | **Research-only named corpus**, isolated from the WI product path. Published Hugging Face parquet https://huggingface.co/datasets/cfahlgren1/sf_criminal_court (`cases.parquet`). Live load 2026-09-16: **77,406** `court_case_raw` rows; `defendant_name` nonempty 77,399/77,406; grain `sf_case:{case_id}`. CC-BY-NC-4.0 — not commercial CRA / employer product. **Do not scrape** SF court sites. **Do not load Cook.** See [`ACCESS_SF_CRIMINAL_HF.md`](ACCESS_SF_CRIMINAL_HF.md). | Published parquet download (`extract_method`: `rest_bulk`) via `bronze/jobs/load_sf_criminal_hf.py`. Landed. |
...
When landing `cases.parquet` (live load 2026-09-16, `ingest_run_id=26a31a80-a06f-4870-ab9e-cda5db38f47f`, 77,406 rows):

- `source_record_id` = `sf_case:{case_id}` when `case_id` is present.
- Else `sf_case:{case_number}`.
- `payload` JSON **must** preserve `defendant_name` and stamp `county=San Francisco`, `locality=SF` (named corpus for `court_party` experiments). Isolated from WI product tables; same Bronze schema, different `source_system` / `state_code`.
...
- `2026-09-16` — Research sources: provisional gated `va_court_data_org` (VA; no anonymized load; Prasanth requests named account) and `sf_criminal_hf` (CA; published HF `cases.parquet`; live load 77,406 rows, grain `sf_case:{case_id}`). See `ACCESS_VA_COURT_DATA.md` and `ACCESS_SF_CRIMINAL_HF.md`. WI WCCA MVP path unchanged.
